	...
	while(pTemp1->pNext != NULL)
	{
		...
		while(pTemp2 != NULL)
		{
			if(pTemp1->at > pTemp2->at)
			{
				...
				pTemp1->p = pTemp1->p + pTemp2->p;
				pTemp2->p = pTemp1->p - pTemp2->p;
				pTemp2->p = pTemp1->p - pTemp2->p;				
			}
...
struct node *get_p(void)
{
	...
	while(pTemp != NULL)
	{
		if(pTemp->at <= time && pTemp->bt1 != 0 && pTemp->p < min)
		{
			min = pTemp->p;
			min_p = pTemp;
		}
...
void output(void)
{
	...
	while(pTemp != NULL)
	{
		...
		printf("\t%s\t%d\t%d\t%d\t%d\t%d\t%d\n", pTemp->name, pTemp->p, pTemp->at, pTemp->bt, pTemp->ct, tat, wt);
